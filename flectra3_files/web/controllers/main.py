...
def ensure_db(redirect='http://localhost:5173'):
    ...
    print('*****Line 164 Request session db Ahora redirige a Mopi: *********', request.session.db)
...
class Home(http.Controller):
    ...
    def web_login(self, redirect='/web', **kw):
        ...
        print('*******Line 530 Resp Headers******:', response.headers)
        print('******Line 531 request.params**********:', request.params)
        session_info = request.env['ir.http'].session_info()
        session_id = session_info['session_id']
        print('+++++++Line 534 web/login session_id+++++++', session_id)
...
class Database(http.Controller):
    ...
    def selector(self, redirect='http://localhost:5173', **kw):
        request._cr = None
        return werkzeug.utils.redirect(redirect, 303)
        # return self._render_template(manage=False)
    # TODO: 
    # - habilitar para signup
    # - reusar la lógica
...
class Session(http.Controller):
    ...
    #Esto permite renderizar la vista de logout
    def _render_template(self, **d):
        return env.get_template("logout.html").render(d)
    
    #TODO:
    # - Redirigir a Mopi si la sesion expiró 
    @http.route('/web/session/get_session_info', type='http', auth="none")
    def get_session_info(self, redirect='http://localhost:5173'):
    ...
    def destroy(self):
        request.session.logout()
 #la lineaa 1041 originalmente redirigía a /web
    @http.route('/web/session/logout', type='http', auth="none")
    def logout(self, redirect=None):
        request.session.logout(keep_db=True)
        r = request.httprequest
        url_logout = werkzeug.urls.url_parse(r.base_url)
        session_info = request.env['ir.http'].session_info()
        request_session_id = session_info['session_id']
        if request.httprequest.method == 'GET':
            print('_________________________________________________________')
            print('request headers:', r.headers)
            request._cr = None
            response = Response(self._render_template(manage=False))
            # # response.headers.add('Set-Cookie', 0)
            # print('response headers:', response.headers)
            # print('Request session id to logout endpoint', request_session_id)
            # print('Response headers logout endpoint:', None)
            print('_________________________________________________________')
            return response
        # return werkzeug.utils.redirect(redirect, 303)
